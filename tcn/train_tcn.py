"""
TCN (Temporal Convolutional Network) Model Training for Seoul Bike Data
Simple script to train TCN model on given train/test data
"""
...
def build_tcn_model(num_features, num_channels=[64, 64, 32], kernel_size=3, dropout=0.2):
...
def train_model(model, train_loader, val_loader, epochs=100, learning_rate=0.001):
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            # Save best model
            torch.save(model.state_dict(), 'tcn/models/best_tcn_model.pth')
        else:
            patience_counter += 1
    ...
    # Load best model
    model.load_state_dict(torch.load('tcn/models/best_tcn_model.pth'))
...
def save_results(train_metrics, test_metrics, history, feature_cols, model_config):
    ...
    with open('tcn/results/tcn_metrics.json', 'w') as f:
        json.dump(results, f, indent=4)

    print("Saved: tcn/results/tcn_metrics.json")
    ...
    history_df.to_csv('tcn/results/training_history.csv', index=False)
    print("Saved: tcn/results/training_history.csv")
    ...
    metrics_df.to_csv('tcn/results/tcn_metrics_summary.csv', index=False)
    print("Saved: tcn/results/tcn_metrics_summary.csv")


def main():
    """Main execution function"""
    print("\n" + "#"*80)
    print("# SEOUL BIKE TCN MODEL TRAINING #")
    print("#"*80 + "\n")

    # Hyperparameters
    SEQUENCE_LENGTH = 24  # Use 24 hours of history
    NUM_CHANNELS = [64, 64, 32]  # TCN channel sizes
    KERNEL_SIZE = 3
    DROPOUT_RATE = 0.2
    EPOCHS = 100
    BATCH_SIZE = 32
    ...
    # Save scalers
    os.makedirs('tcn/models', exist_ok=True)
    joblib.dump(scaler, 'tcn/models/feature_scaler.pkl')
    joblib.dump(target_scaler, 'tcn/models/target_scaler.pkl')
    print("\nSaved scalers:")
    print("  tcn/models/feature_scaler.pkl")
    print("  tcn/models/target_scaler.pkl")

    # Save final model
    torch.save(model.state_dict(), 'tcn/models/tcn_model.pth')
    print("\nSaved final model:")
    print("  tcn/models/tcn_model.pth")
